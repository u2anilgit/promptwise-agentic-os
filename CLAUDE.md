...
Phase 0 complete: compose foundation (Ollama + Qdrant + FastAPI gateway + `promptwise` CLI + doctor) is implemented and verified live via `docker compose up`. Default gateway host port is `8420` (see `compose/.env.example`). Phase 1 complete: hybrid router (`route_request`, RAM watchdog, config-driven tier order), model catalog loader with packaged fallback, and `$/completed-task` cost report are implemented and tested. Phase 2 (Verification Gate) is partially implemented (`core/verify/` — gate/ledger/runners/security) but not yet confirmed against `docs/ROADMAP.md`'s acceptance criteria. Pack-loader foundation (`core/packs/` — manifest schema/loader/registry, new `intelligence` pack kind, `promptwise pack install/list/remove`, real `packs.integrity` doctor validation) is implemented and tested — this is the Phase-8-gated slice specified in `docs/superpowers/specs/2026-08-24-repo-intelligence-methodology-packs-design.md`; repo-intelligence pack content and BMAD/DMAIC methodology packs remain blocked on Phases 3–5. Next action: write the Phase 2 plan (Verification Gate) per `docs/ROADMAP.md`, or confirm/close out the existing `core/verify/` work against its acceptance criteria.
